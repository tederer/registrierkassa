...
describe('BusBridge', function() {
   ...
   it('the data of a received publication do not get sent back to the bus that originally sent the publication message', function() {
      var topic = '/bidirectional/topic';
      var data = 27;
      var message = common.infrastructure.busbridge.MessageFactory.createPublicationMessage(topic, data);
      
      givenABusBridgeWhichTransmits([topic]);
      
      mockedBus.invokeAllRegisteredCallbacksWhenPublishGetsCalled();
      mockedConnection.simulateReceivedMessage(message);
      
      expect(mockedConnection.capturedSentMessages.length).to.be.eql(0);
   });
   ...
   it('the data of a received command do not get sent back to the bus that originally sent the command message', function() {
      var topic = '/bidirectional/command';
      var data = 'I do not want to get sent back';
      var message = common.infrastructure.busbridge.MessageFactory.createCommandMessage(topic, data);
      
      givenABusBridgeWhichTransmits([topic]);
      
      mockedBus.invokeAllRegisteredCallbacksWhenSendCommandGetsCalled();
      mockedConnection.simulateReceivedMessage(message);
