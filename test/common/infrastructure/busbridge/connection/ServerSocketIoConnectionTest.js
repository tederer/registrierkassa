...
describe('ServerSocketIoConnection', function() {
   ...
   it('when a socket.io socket emits a message event, then the message gets forwarded to all other connected sockets', function() {
      var data = {name: 'Trick', species: 'Duck'};
      ...
      expect(anotherMockedSocket.capturedEmittedMessages.length).to.be.eql(1);
      expect(anotherMockedSocket.capturedEmittedMessages[0].eventtype).to.be.eql('message');
      expect(anotherMockedSocket.capturedEmittedMessages[0].data).to.be.eql('{"name":"Trick","species":"Duck"}');
   });
